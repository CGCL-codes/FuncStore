...
void S3Client::put_object( const std::string & bucket,
                            ...
                            const std::string & content)
{
  ...
  S3PutRequest request { credentials_, endpoint, config_.region, object, content };
  HTTPRequest outgoing_request = request.to_http_request();
  responses.new_request_arrived( outgoing_request );
  s3.write( outgoing_request.str() );
  ...
  if ( responses.front().first_line() != "HTTP/1.1 200 OK" ) {
    throw runtime_error( "HTTP failure in S3Client::put_object( " + bucket + ", " + object + " ): " + responses.front().first_line() );
  }
}
  
std::string S3Client::get_object( const std::string & bucket,
                            const std::string & object)
{
  const string endpoint = ( config_.endpoint.length() > 0 )
                          ? config_.endpoint : S3::endpoint( config_.region, bucket );
  ...
  if ( responses.front().first_line() != "HTTP/1.1 200 OK" ) {
    throw runtime_error( "HTTP failure in S3Client::get_object( " + bucket + ", " + object + " ): " + responses.front().first_line() );
  }
  else {
    return std::move(responses.front().body());
  }
}
